          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-widest text-cyan-300">
                {result.selected.platform} / {result.selected.productId}
              </p>
              <h2 className="text-xl font-semibold text-slate-100">{result.selected.productName || result.selected.productId}</h2>
            </div>
            <Link href={`/title/${result.selected.platform}/${result.selected.productId}`} className="btn-gold">
              作品ページを見る
            </Link>
          </div>
        </section>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {result.items.map((item) => (
              <Link
                ...
                href={`/title/${item.platform}/${item.productId}`}
