...
export type Platform = "fanza" | "fantia";
...
export type ReviewResult = {
  parsed: { productId: string; platform: Platform } | null;
  reviews: Review[];
  summary: ReviewSummary;
};

function parseReviewUrl(url: string): { productId: string; platform: Platform } | null {
...
export async function initDatabase() {
  ...
  const countResult = await sql<{ count: string }>`SELECT COUNT(*)::text AS count FROM reviews`;
...
async function getReviewsInternal(productId: string, platform: Platform): Promise<ReviewResult> {
  await initDatabase();
...
export async function getReviewsByUrl(url: string): Promise<ReviewResult> {
...
export async function submitReview(input: {
  ...
  comment?: string;
  tags: string[];
}): Promise<{ ok: boolean; message: string; parsed: { productId: string; platform: Platform } | null }> {
